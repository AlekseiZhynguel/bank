...
public class VerifyAccountsExistOnTransactionSent {
    ...
    public void on(TransactionSent event) {
        if(bothAccountsExistFrom(event)) {
            eventBus.publish(Collections.singletonList(new AccountsVerifiedOnTransactionSent(event.aggregateId(), event.originAccountId(), event.destinationAccountId(), event.amount())));
        }
    }
    ...
    private boolean bothAccountsExistFrom(TransactionSent event) {
        ...
        return finder.existsById(origin)
                && finder.existsById(destination);
    }
}
